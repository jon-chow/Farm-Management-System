 ...
 */
export const retrieveFilteredLivestock = async (
  filteredData: FilteredLivestock
) => {
...
export const updateLivestock = async (
  ...
  const res = await axios.post(
    `${PATH}/update`,
    { actionType: action, livestock: livestockJson },
    {
      headers: {
        "Content-Type": "application/json",
      },
  ...
  if (res.data) return res.data;
    ...
    throw new Error(
      `Failed to update livestock with tagID #${livestock.tagID}!`
    );
};
...
/**
 * Retrieves the count of all livestock grouped by animal type
 */
export const getLivestockCount = async () => {
  const res = await axios.get(`${PATH}/get/animalCount`, {});

  if (res.data) return res.data;
  else throw new Error(`Failed to retrieve counts of animals!`);
};
